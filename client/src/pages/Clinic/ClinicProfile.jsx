...
import { 
  ...
  Image as ImageIcon, Stethoscope, Save, X, RefreshCw,
...
const ALL_SERVICES = ['General Consultation', 'Vaccination', 'Surgery', 'Dental Care', 'X-Ray & Ultrasound', 'Laboratory Tests', 'Emergency Care', 'Microchipping'];
const ALL_FACILITIES = ['Parking', 'Air Conditioned', 'Waiting Lounge', 'Operation Theater', 'In-house Pharmacy', 'Emergency Ward', 'CCTV Monitoring', 'Separate Cat Area'];
...
const ClinicProfile = () => {
  ...
  const [profile, setProfile] = useState({
    ...
    weeklySchedule: DAYS.reduce((acc, day) => ({...acc, [day]: { open: '09:00', close: '20:00', isClosed: false }}), {}),
    services: [], pricing: [], facilities: [],
    socialLinks: { instagram: '', facebook: '', website: '', whatsapp: '' },
    rating: 5.0, reviewCount: 0, yearsExperience: 5, isVerified: false,
    isActive: true, gallery: []
  });

  const [stats, setStats] = useState({
    todayPatients: 24, completed: 18, pending: 5, cancelled: 1,
    revenue: '$4,200', rating: 5.0, avgTime: '20 mins'
  });
  ...
  const getImageUrl = (imagePath, fallbackType) => {
    if (!imagePath || imagePath === 'default.jpg' || imagePath === 'default-logo.jpg' || imagePath === 'default-cover.jpg') {
      return fallbackType === 'cover' ? '/src/assets/images/boarding-cover.png' : `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Clinic')}`;
    }
    if (imagePath.startsWith('http') || imagePath.startsWith('data:')) return imagePath;
    
    if (imagePath.startsWith('clinic') && imagePath.includes('.')) {
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const res = await api.put('/clinic/profile', profile);
      if (res.data.success) {
          ...
          <div className="absolute -top-20 left-8">
            <div className="w-40 h-40 bg-white rounded-full p-1.5 shadow-xl">
              <div className="w-full h-full bg-white rounded-full flex items-center justify-center border-4 border-white overflow-hidden relative group/logo">
                <img 
                  src={logoImg || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Clinic')}`} 
                  alt="Logo" 
                  className="w-full h-full object-cover" 
                  onError={(e) => { 
                    const fallbackUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Clinic')}`;
          ...
          <div className="ml-48 flex justify-between items-start mt-2">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{profile.name || 'Veterinary Clinic'}</h1>
                {profile.isVerified && (
                  <span className="bg-green-50 text-green-700 px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 border border-green-200 shadow-sm">
                    <ShieldCheck size={14} /> Verified Clinic
                  </span>
                )}
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-6">
              <Building2 size={20} className="text-primary" /> Clinic Information
            </h2>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
              <Info size={20} className="text-primary" /> About Clinic
            </h2>
            <textarea 
              rows={4} 
              placeholder="Tell pet parents about your veterinary clinic, doctors, experience, and specialties..."
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-6">
              <Stethoscope size={20} className="text-primary" /> Services & Consultations
            </h2>
            ...
            <div className="border-t border-gray-100 pt-6">
               <div className="flex justify-between items-center mb-4">
                 <label className="block text-sm font-medium text-gray-700">Consultation Pricing</label>
                 <button 
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
              <ShieldCheck size={20} className="text-primary" /> Clinic Facilities
            </h2>
          ...
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-lg p-6 text-white relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 relative z-10">
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                <div className="text-white/60 text-xs font-medium mb-1">Patients</div>
                <div className="text-2xl font-bold">{stats.todayPatients}</div>
              </div>
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                <div className="text-white/60 text-xs font-medium mb-1">Consulted</div>
                <div className="text-2xl font-bold text-green-400">{stats.completed}</div>
              </div>
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                <div className="text-white/60 text-xs font-medium mb-1">Waiting</div>
                  ...
                  {!profile.weeklySchedule[day]?.isClosed ? (
                    <div className="flex items-center gap-2">
                      ...
                      <input 
                        type="time" 
                        className="text-sm border border-gray-200 rounded-lg px-2 py-1 bg-gray-50 focus:bg-white outline-none"
                        value={profile.weeklySchedule[day]?.close || '20:00'}
                        onChange={(e) => {
                          setProfile({
                            ...profile, 
                            weeklySchedule: {
