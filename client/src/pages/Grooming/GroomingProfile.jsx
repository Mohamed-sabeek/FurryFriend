...
import { 
  ...
  Image as ImageIcon, Scissors, Save, X, RefreshCw,
...
const ALL_SERVICES = ['Bathing', 'Hair Trimming', 'Nail Clipping', 'Ear Cleaning', 'Teeth Cleaning', 'Flea Treatment', 'Spa', 'Styling'];
const ALL_FACILITIES = ['Parking', 'Air Conditioned', 'Pet Play Area', 'Waiting Lounge', 'Pickup & Drop', 'Emergency Care', 'CCTV Monitoring', 'Separate Cat Area'];
...
const GroomingProfile = () => {
  ...
  const [profile, setProfile] = useState({
    ...
    weeklySchedule: DAYS.reduce((acc, day) => ({...acc, [day]: { open: '09:00', close: '19:00', isClosed: false }}), {}),
    services: [], pricing: [], capacity: { petsPerDay: 20 }, facilities: [],
    socialLinks: { instagram: '', facebook: '', website: '', whatsapp: '' },
    rating: 4.8, reviewCount: 0, yearsExperience: 1, isVerified: false,
    isActive: true, gallery: []
  });

  const [stats, setStats] = useState({
    todayGroomings: 14, completed: 8, pending: 4, cancelled: 2,
    revenue: '$1,250', rating: 4.8, avgTime: '45 mins'
  });
  ...
  const getImageUrl = (imagePath, fallbackType) => {
    if (!imagePath || imagePath === 'default.jpg' || imagePath === 'default-logo.jpg' || imagePath === 'default-cover.jpg') {
      return fallbackType === 'cover' ? '/src/assets/images/boarding-cover.png' : `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Grooming')}`;
    }
    if (imagePath.startsWith('http') || imagePath.startsWith('data:')) return imagePath;
    
    if (imagePath.startsWith('grooming') && imagePath.includes('.')) {
      // Check if it's actually in assets, otherwise use boarding cover
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      const res = await api.get('/grooming/center/profile');
      if (res.data.success && res.data.data) {
        setProfile(prev => ({
          ...prev,
          ...res.data.data,
          weeklySchedule: { ...prev.weeklySchedule, ...(res.data.data.weeklySchedule || {}) },
          capacity: { ...prev.capacity, ...(res.data.data.capacity || {}) },
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      const res = await api.put('/grooming/center/profile', profile);
      if (res.data.success) {
  ...
  const calculateCompletion = () => {
    ...
    const check = (val) => { fields++; if (val && val.length !== 0) filled++; };
    
    check(profile.name); check(profile.phone); check(profile.address); check(profile.city);
    check(profile.description); check(profile.services?.length > 0); check(profile.pricing?.length > 0);
    check(profile.logo); check(profile.coverImage);
    ...
    <div className="max-w-6xl mx-auto space-y-6 relative">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative group">
        <div className="h-72 bg-gray-900 relative rounded-t-2xl overflow-hidden">
          <img 
            src={coverImg || '/src/assets/images/boarding-cover.png'} 
            alt="Cover" 
            className="w-full h-full object-cover transition-opacity duration-500 ease-in-out" 
            onError={(e) => { 
              if (e.target.src !== '/src/assets/images/boarding-cover.png') {
                e.target.src = '/src/assets/images/boarding-cover.png'; 
              }
            }}
          />
          ...
          <div className="absolute -top-20 left-8">
            <div className="w-40 h-40 bg-white rounded-full p-1.5 shadow-xl">
              <div className="w-full h-full bg-white rounded-full flex items-center justify-center border-4 border-white overflow-hidden relative group/logo">
                <img 
                  src={logoImg || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Grooming')}`} 
                  alt="Logo" 
                  className="w-full h-full object-cover" 
                  onError={(e) => { 
                    const fallbackUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(profile?.name || 'Grooming')}`;
          ...
          <div className="ml-48 flex justify-between items-start mt-2">
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{profile.name || 'Grooming Shop'}</h1>
                {profile.isVerified && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            ...
            <textarea 
              rows={4} 
              placeholder="Tell pet parents about your grooming center, experience, and what makes you special..."
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-6">
              <Scissors size={20} className="text-primary" /> Services & Pricing
            </h2>
            ...
            <div className="border-t border-gray-100 pt-6">
               <div className="flex justify-between items-center mb-4">
                 <label className="block text-sm font-medium text-gray-700">Service Pricing</label>
                 <button 
                 ...
                 {profile.pricing?.map((pkg, idx) => (
                   <div key={idx} className="flex gap-3 items-center bg-gray-50 p-2 rounded-xl border border-gray-100">
                     <div className="flex-1 relative">
                       <input 
                        type="text" 
                        placeholder="E.g. Small Dog Bath"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-lg p-6 text-white relative overflow-hidden">
            ...
            <h2 className="text-lg font-bold flex items-center gap-2 mb-6 relative z-10">
              <Star size={20} className="text-yellow-400" /> Today's Overview
            </h2>
            <div className="grid grid-cols-2 gap-4 relative z-10">
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
                <div className="text-white/60 text-xs font-medium mb-1">Groomings</div>
                <div className="text-2xl font-bold">{stats.todayGroomings}</div>
              </div>
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm border border-white/10">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-4">
              <Users size={20} className="text-primary" /> Daily Capacity
            </h2>
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex items-center justify-between">
              <div>
                <div className="text-sm font-medium text-gray-700">Pets Per Day</div>
                <div className="text-xs text-gray-500">Max limit for bookings</div>
              </div>
              <input 
                type="number" 
                className="w-20 px-3 py-2 border border-gray-200 rounded-lg text-center font-bold focus:border-primary outline-none"
                value={profile.capacity?.petsPerDay || 0}
                onChange={e => setProfile({...profile, capacity: { ...profile.capacity, petsPerDay: Number(e.target.value) }})}
              />
            </div>
            <div className="mt-3 flex justify-between text-sm px-1">
              <span className="text-gray-500">Current Bookings: <span className="font-bold text-gray-900">{stats.todayGroomings}</span></span>
              <span className="text-gray-500">Available: <span className="font-bold text-green-600">{Math.max(0, (profile.capacity?.petsPerDay || 0) - stats.todayGroomings)}</span></span>
            </div>
          </div>
